...
function hitChrome(c) {
  ...
  function done() {
    ...
    client.Page.navigate({ url: 'https://google.se' }) // Needed so next run won't preserve some log msgs
    client.close()
    die('', 0)
  }
...
if (argv.h || argv.help) {
  ...
  return die('  ❌ Error: Need some pages to test\n\n' + help, 1)
} else if (profile && profile !=='3gDsl' && !Object.keys(networkProfiles).includes(profile)) {
  return die('  ❌ Error: Invalid profile\n\n' + help, 1)
} else if (urls.find( url => !url.includes('http') )){
  return die('  ❌ Error: Please enter full url (including http(s)://)\n\n' + help, 1)
}
